...
if security find-identity -p codesigning 2>/dev/null | grep -q "$IDENTITY"; then
  codesign --force --sign "$IDENTITY" --identifier local.frontier "$BUNDLE" 2>&1 | grep -v "replacing existing" || true
else
  codesign --force --sign - "$BUNDLE" >/dev/null 2>&1
fi

echo "==> Installing to /Applications"
# Every copy, not just the installed one. Anchoring this to /Applications left
# a second app running from the staging bundle below — same bundle id, same data
# directory, and after a rebuild the survivor was the older build.
...
echo "Done. Concepts live in ~/Library/Application Support/Frontier/concepts."
